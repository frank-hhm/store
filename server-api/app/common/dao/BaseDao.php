...
abstract class BaseDao
{
     ...
     * 获取某些条件数据
     */
    public function getPageList(array $where,int $page = 0,int $limit = 0,array $with = [],$order = ['id DESC'], $isArr = true, $field = '*'): \think\Paginator|array
    {
        if($limit == 0){
     ...
     * 获取某些条件数据
     */
    public function selectList(array $where,int $page = 0,int $limit = 0,array | string $field = '*'): \think\Collection|array
    {
     ...
     * 数据详细
     */
    public function detail($filter,?array $with = [], array|string $field = '*'){
        if(is_array($filter)){
     ...
     * 获取分页配置
     */
    public function getPageValue(bool $isPage = true, bool $isRelieve = false): array
    {
        $page = $limit = 0;
        if ($isPage) {
