    ...
    <layout-body-content pageHeader>
        ...
        <template v-slot:content="{
            ...
            <a-table :loading="initLoading" :data="lists" row-key="id" isLeaf :pagination="false" :scroll="{
                ...
                <template #columns>
                    <a-table-column title="ID" align="center" data-index="id" :width="100">
                        <template #cell="{ record }">
                            <div>{{ record.id }}</div>
                        </template>
                    </a-table-column>
                    <a-table-column title="标签名称" data-index="label_name">
                        <template #cell="{ record }">
                            <div>{{ record.label_name }}</div>
                        </template>
                    </a-table-column>
                    ...
                    <a-table-column title="操作" align="center" :width="140">
                        <template #cell="{ record }">
                            <a-space>
                                <a-button @click=" onCreate(record.id)" v-permission="'store-goods_label-update'"
                                    size="small">编辑</a-button>
                                <div v-permission="'store-goods_label-delete'">
...
import createGoodsLabelComponent from './create.vue';
import { useSetting } from "@/hooks/useSetting";
import { PageLimitType, Result, ResultError } from "@/types";
import { deleteStoreGoodsLabelApi, getStoreGoodsLabelListApi } from "@/api/store/goods-label";
...
const createGoodsLabelComponentRef = ref<HTMLElement>()

const onCreate = (id: number) => {
    proxy?.$refs['createGoodsLabelComponentRef'].open(id)
}
